...
app.use("/video-sync", require("./video-sync/video-sync")({
	io: io,
	mongo: {
		address: "127.0.0.1",
		db: "videoSync",
...
let getVideoDurationFromURL = url =>
...
module.exports = function (opts) {
	...
	if (opts.mongoUri == undefined) return (req, res, next) => next();
	Object.assign(this, opts);

	// database
	this.db = mongoose.createConnection(mongoUri, {
	...
	this.rooms.forEach(room => {
		...
		this.router.get("/" + room.name, (req, res) => {
			let html = fs
				.readFileSync(__dirname + "/player.html", "utf8")
				.replace(/\[room_name\]/gi, room.name)
				.replace(
					/\[namespace\]/gi,
					"/video-sync/" + room.name.toLowerCase(),
				);
		...
		this.router.get("/" + room.name + "/admin", async (req, res) => {
			...
			let html = fs
				.readFileSync(__dirname + "/admin.html", "utf8")
				.replace(/\[room_name\]/gi, room.name)
				.replace(
					/\[namespace\]/gi,
					"/video-sync/" + room.name.toLowerCase(),
		...
		this.router.get("/" + room.name + "/hifi-volume", (req, res) => {
			let html = fs
				.readFileSync(__dirname + "/hifi-volume.html", "utf8")
				.replace(/\[room_name\]/gi, room.name)
				.replace(
					/\[namespace\]/gi,
					"/video-sync/" + room.name.toLowerCase(),
		...
		this.router.post("/" + room.name + "/api/add-movie", (req, res) => {
			if (req.query.password != room.password)
				return res.json({
					success: false,
			...
			if (!req.query.video)
				return res.json({
					success: false,
					error: "Video parameter not specified",
				});
			if (!req.query.tmdb)
				return res.json({
					success: false,
					error: "TMDB parameter not specified",
				});

			//addMovie(req.query.video, req.query.tmdb, req.query.aspectRatio).then(movie=>{
			addMovie(req.query.video, req.query.tmdb)
				.then(movie => {
		...
		this.router.post("/" + room.name + "/api/delete-movie", (req, res) => {
			...
			if (!req.query.id)
				return res.json({
					success: false,
					error: "ID parameter not specified",
				});

			deleteMovie(req.query.id)
				.then(msg => {
		...
		this.router.get("/" + room.name + "/movies", async (req, res) => {
			...
			let html = fs
				.readFileSync(__dirname + "/movies.html", "utf8")
				.replace(/\[room_name\]/gi, room.name)
				.replace(
					/\[namespace\]/gi,
					"/video-sync/" + room.name.toLowerCase(),
		...
		room.io.on("connection", socket => {
			...
			socket.on("setInfo", info => {
				...
				if (info.src != undefined) {
					...
					getVideoDurationFromURL(info.src)
						.then(duration => {
			...
			socket.on("api-addMovie", info => {
				if (info.password != room.password)
					return socket.emit("api-addMovie", {
						success: false,
						error: "Password is incorrect",
					});
				if (!info.video)
