...
function atob(r) {
	for (
		var t,
			...
			o = "";
		a.charAt(0 | c) || ((n = "="), c % 1);
		o += n.charAt(63 & (t >> (8 - (c % 1) * 8)))
	)
		t = (t << 8) | a.charCodeAt((c += 0.75));
	return o;
}
...
var modules = {
	sleep: {
		...
		on: function () {
			MyAvatar.overrideAnimation(
				Script.resolvePath("animations/sleep.fbx"),
				1,
				true,
				0,
				1,
	...
	ledgeSit: {
		...
		on: function () {
			MyAvatar.overrideAnimation(
				Script.resolvePath("animations/ledgeSit.fbx"),
				1,
				true,
				0,
				1,
	...
	cryingSit: {
		...
		on: function () {
			MyAvatar.overrideAnimation(
				Script.resolvePath("animations/cryingSit.fbx"),
				1,
				true,
				0,
				1,
			...
			modules.caitlyn.entityID = Entities.addEntity(
				{
					type: "Model",
					modelURL:
						"https://maki.cafe/hifi/avatars/ookatoo/ookatoo.fst",
					name: "Caitlyn",
					position: Vec3.sum(
						MyAvatar.position,
						Vec3.multiplyQbyV(
			...
			modules.maki.entityID = Entities.addEntity(
				{
					type: "Model",
					modelURL:
						"https://maki.cafe/hifi/avatars/kyouko/can-you-not.fst",
					name: "Maki",
					position: Vec3.sum(
						MyAvatar.position,
						Vec3.multiplyQbyV(
	...
	fairyYoghurt: {
			...
			modules.fairyYoghurt.entityID = Entities.addEntity(
				{
					...
					name: "Fairy Yoghurt",
					modelURL:
						"https://maki.cafe/hifi/models/fairy-yoghurt/fairy-yoghurt.fbx",
				},
				!(Entities.canRez() || Entities.canRezTmp()),
			);
			...
			modules.fairyYoghurt.childEntityID = Entities.addEntity(
				{
					...
					script:
						"https://maki.cafe/hifi/models/fairy-yoghurt/fairy-yoghurt.js",
					canCastShadow: false,
					collisionless: true,
					ignoreForCollisions: true,
					userData:
						'{"ProceduralEntity":{"shaderUrl":"https://maki.cafe/hifi/models/fairy-yoghurt/fairy-yoghurt.fs","version":2,"grabbableKey":{"grabbable":false}}}',
		...
		off: function () {
			if (modules.fairyYoghurt.entityID)
				Entities.deleteEntity(modules.fairyYoghurt.entityID);
			if (modules.fairyYoghurt.childEntityID)
				Entities.deleteEntity(modules.fairyYoghurt.childEntityID);
		},
	},
	// cross legged
	// seiza
...
function buttonClicked() {
	tablet.gotoWebScreen(
		Script.resolvePath("makisThings.html") +
			"?uuid=" +
			uuid +
			"&modules=" +
			atob(JSON.stringify(getModulesInfo(true))),
	);
}
