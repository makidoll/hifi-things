...
//var threeDeeTextURL = "https://maki.cafe/3d-text.obj?font=apple_kid&scale=0.01&depth=0.05&frontEmission=ffffff&sideEmission=E91E63&text=";
var threeDeeTextURL =
	"https://maki.cafe/3d-text.obj?font=apple_kid&scale=0.01&depth=0.05&frontEmission=ffffff&text=";
...
function drawNametag(avatarID) {
	...
	var overlay = Overlays.addOverlay("model", {
		...
		position: Vec3.sum(avatar.position, {
			x: 0,
		...
		rotation: Quat.multiply(
			Quat.cancelOutRollAndPitch(avatar.orientation),
		...
		url: threeDeeTextURL + displayName,
...
function deleteNametag(id) {
	...
	Overlays.deleteOverlay(nametags[id].overlay);
	delete nametags[id];
}
...
function reloadNametags() {
	if (config.debug)
		debug("Reloading " + Object.keys(nametags).length + " nametags");
	...
	avatars.forEach(function (avatarID) {
		...
		if (avatar.sessionDisplayName != nametag.displayName) {
			...
			Overlays.editOverlay(nametag.overlay, {
				url: threeDeeTextURL + nametag.displayName,
			});
		}
	});
	...
	if (config.debug)
		debug("\tReloaded, now " + Object.keys(nametags).length + " in total");
}
...
var button = tablet.addButton({
	icon:
		'data:image/svg;xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#fff"><path d="M19 3h-4.18C14.4 1.84 13.3 1 12 1c-1.3 0-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm0 4c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm6 12H6v-1.4c0-2 4-3.1 6-3.1s6 1.1 6 3.1V19z"/></svg>',
	text: "Nametags",
});
...
function buttonClicked() {
	...
	tablet.gotoWebScreen(
		Script.resolvePath("makisNametags.html") +
			"?uuid=" +
			uuid +
			"&config=" +
			atob(JSON.stringify(getConfig())),
	);
}
