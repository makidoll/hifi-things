...
function spawnPortal(place) {
	var position = Vec3.sum(
		...
		Vec3.multiplyQbyV(Quat.cancelOutRollAndPitch(Camera.orientation), {
			y: 1.5,
			z: -2,
		}),
	);
	...
	var parentID = Entities.addEntity({
		...
		position: position,
		rotation: rotation,
		script: "https://hifi.maki.cafe/client-entity-scripts/portal.js",
		collisionless: true,
		...
		userData: JSON.stringify({
			...
			ProceduralEntity: {
				shaderUrl: "https://hifi.maki.cafe/shaders/portal.fs",
		...
		case "getPlaceInfo":
			...
			getPlaceInfo(json.value, function (body) {
				var place = {
					...
					desc: /<p class=['"]places-regular-text places-left-justify['"]>(.*?)<\/p>/i.exec(
						body,
					),
					thumbURL: /<img class=['"]places-img['"] src="(.*?)[?'"]/i.exec(
						body,
					),
					people: /([0-9]{1,10}) (?:people|person) here<\/span>/i.exec(
						body,
					),
				};
				...
				request(
					"https://maki.cafe/average-color?url=" + place.thumbURL,
					function (body) {
