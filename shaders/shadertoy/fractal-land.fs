/*
shader from https://www.shadertoy.com/view/XsBXWt
on a quad
{
	"ProceduralEntity": {
		"version": 3,
		"shaderUrl": "https://hifi.maki.cafe/shaders/shadertoy/fractal-land.fs",
		"channels": ["https://hifi.maki.cafe/shaders/shadertoy/nyan-cat.png"]
	}
}
*/
...
#define GAMMA 1.4 * 1.7
#define SATURATION .65 * 1.3
...
vec4 nyan(vec2 p)
{
	...
	vec4 color = texture(iChannel0,clamp(vec2(uv.x/3.0+210.0/256.0-nt+0.05,.5-uv.y-ny),0,1));
...

{
	...
	float sunsize=7.-max(0.,(sin(iGlobalTime*3)*0.5+0.5)*0.2)*5.; // responsive sun size
	...
	if (totdist>8.) col=mix(col,rain.xyz,rain.a*.9);
...
float getProceduralFragment(inout ProceduralFragment frag) {
	vec2 uv = _position.xz * 2;
	vec2 oriuv = uv;
	uv.y *= (iWorldScale.z/iWorldScale.x);
	// uv.y *= 2;

	float fov = 0.9 - max(0.0, 0.7 - iGlobalTime * 0.3);
