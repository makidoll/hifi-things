...
vec3 getColor(vec2 uv) {
	...
	float n = pnoise(vec3(
			uv.x*16,
			uv.y*16 - iGlobalTime*2,
			iGlobalTime*1
	)); // -1 to 1

	// image
	vec2 cropImageUV = uv*1.2;
	cropImageUV.x += n*0.1;
	float cropImageLength = length(cropImageUV*2); // 0 to 1
	
	if (cropImageLength<0.5) {
		vec2 imageUV = uv;
		imageUV.x += n*0.02;
		return texture(iChannel0, imageUV*2.2 + 0.5).rgb;
	}

	// edges
	float l = length(uv*2); // 0 to 1
	if (l*2-(n/2+0.5) > 1) discard;
...
float scene(vec3 p, vec3 xzDir) {
	p.xz = rotate(p.xz, atan(xzDir.x, xzDir.z));
	return box(p, vec3(0.5,0.5,0));
}
...
vec3 raymarch(vec3 rayOrigin, vec3 rayDir) {
	...
	for (raySteps=0; raySteps<maxSteps; raySteps++) { 
		float dist = scene(rayPos, rayDir);
		rayPos += rayDir*dist;
